...
def verdict_of(row):
    ...
    #
    # 2026-09-18 措辞修复：原先这一档叫「★ 需求明确、市场空白（最高优先）」。
    # 但判据 mp == 0 来自 github_mature_count（只统计 GitHub stars>1000 的**开源**项目），
    # 闭源商业 SaaS 根本不在视野里 —— 大多数赚钱的产品恰恰是闭源的。
    # 而且该数字还强烈依赖关键词选择（见 sources.github_count 的注释）。
    # 所以 0 只能读作"开源侧没人做大"，读成"市场空白"是双重过度解读。
    # 标签与理由都改成准确表述，并把局限直接写进理由里（不能只留在文档里）。
    if ev >= 2 and heat >= 200 and mp == 0 and (market or 0) < 300:
        return ("★ 需求明确、开源侧无成型产品（最高优先）", 5,
                [f"讨论热度 {heat}（评论赞同+评论数加权）",
                 f"证据 {ev} 条 · {div} 个平台",
                 "开源侧没有成型产品（GitHub stars>1000 计数为 0）",
                 "⚠ 该计数**只看开源**，闭源商业 SaaS 不在视野内；"
                 "且受关键词选择影响 —— 0 只等于「开源侧没人做成」，不等于市场空白"],
                "去高热原帖评论区找 3 个抱怨者聊；"
                "**并必须单独核查该方向的闭源商业产品**（GitHub 看不到这一块）")
