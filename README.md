...
产出 `out/leads_*.html`（**主交付**，单文件离线可开、零外部依赖）+ `.md` + `.json`：

| 产出 | 用途 |
|---|---|
| `leads_*.html` | **主交付**。对着屏幕一条条读、点开链接、复制话术 —— 卡片式 + 判断筛选 + 一键复制 |
| `leads_*.md` | 归档与 diff |
| `leads_*.json` | 机器消费（含结构化话术） |
| `leads_cache_*.json` | 供 `--from-cache` 复用的采集缓存 |

HTML 里有什么：**谁 / 要做什么 / 预算 / 原文链接 / 怎么联系 / 值不值得联系**，
每条附可复制的应征话术与针对性提醒，末尾是已知局限与通道健康。
